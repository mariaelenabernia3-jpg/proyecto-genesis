document.addEventListener('DOMContentLoaded', () => {
    ...
    function requestSave() {
        // Cancela cualquier guardado pendiente para agruparlo con este nuevo.
        clearTimeout(saveTimeout);
        // Establece un nuevo guardado para dentro de 2 segundos.
        // Si se realizan muchas acciones rápidas, solo se ejecutará el último.
    ...
    function getDefaultState() { /* (código sin cambios) */ }
    function updateUI() { /* (código sin cambios) */ }

    // --- ACCIONES DEL JUGADOR (CON GUARDADO POR EVENTOS) ---
    window.sellMaterial = (key, amount) => {
        ...
        if (gameState.inventory[key] >= amountToSell && amountToSell > 0) {
            ...
            const modifier = CONFIG.PLANETS[gameState.currentPlanet].priceModifiers[key];
            const price = material.baseValue * modifier;
            gameState.inventory[key] -= amountToSell;
            gameState.money += price * amountToSell;
            requestSave(); // << GUARDADO POR EVENTO
        }
    };

    window.buyUpgrade = (key) => {
        const upgrade = CONFIG.UPGRADES[key];
        const cost = Math.ceil(upgrade.cost * Math.pow(1.15, gameState.upgradeLevels[key]));
    ...
    window.travelToPlanet = (key) => {
        if (gameState.currentPlanet !== key) {
            gameState.currentPlanet = key;
            requestSave(); // << GUARDADO POR EVENTO
        }
    };

    // --- LÓGICA DEL JUEGO Y BUCLES ---
    function calculateProduction() { /* (código sin cambios) */ }
    function gameLoop() {
    ...
    // --- INICIALIZACIÓN DE BUCLES Y EVENTOS DE GUARDADO ---
    setInterval(gameLoop, 100); // Bucle principal del juego
    setInterval(requestSave, 15000); // Guardado periódico (cada 15s)
    window.addEventListener('beforeunload', saveGame); // Guardado final al cerrar la página


    // --- INCLUSIÓN DE FUNCIONES NO MODIFICADAS PARA QUE ESTÉ COMPLETO ---
    ...
    function updateUI() {
        ...
        inventoryListEl.innerHTML = Object.entries(gameState.inventory).map(([key, value]) => `<div class="resource-line"><span class="item-name">${CONFIG.MATERIALS[key].name}</span><span class="item-details">${Math.floor(value).toLocaleString()}</span></div>`).join('');
